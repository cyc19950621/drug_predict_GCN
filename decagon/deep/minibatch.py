...
class EdgeMinibatchIterator(object):#mini batch 梯度下降
    """ This minibatch iterator iterates over batches of sampled edges or
    random pairs of co-occuring edges. 对共线的边进行随机梯度下降
    ...
    def preprocess_graph(self, adj):
        ...
        if adj.shape[0] == adj.shape[1]: #如果是对称矩阵
            adj_ = adj + sp.eye(adj.shape[0]) #生成单位矩阵并和之前叠加
            rowsum = np.array(adj_.sum(1)) #矩阵相加
            degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -0.5).flatten())#构造对角矩阵（含偏移量）返回一维数组
            adj_normalized = adj_.dot(degree_mat_inv_sqrt).transpose().dot(degree_mat_inv_sqrt).tocoo()#.dot矩阵积
        else:
    ...
    def _ismember(self, a, b):#判断输入的AB是否相同
        a = np.array(a)
        b = np.array(b)
        rows_close = np.all(a - b == 0, axis=1)#横向传播
        return np.any(rows_close) #返回布尔值
